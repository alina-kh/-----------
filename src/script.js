...
let started = false; //таймер не запущен

function start() {
  if (started) {return}; // если таймер запущен — выходим из функции

  let startTime = new Date();   //сохраняем время нажатия
  // 
  ...
  started = true; // помечаем, что таймер уже запущен
}